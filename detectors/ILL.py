...
def import_nomad_list(self, file_numbers, time_file, data_folder):

    print('### Importing list data files of file numbers: ' + str(file_numbers[0]) + ' to ' + str(file_numbers[-1]))
    ...
    # if time_file='none', all files will be assigned the same timestamp corresponding to the time of the analysis execution
    if time_file == 'none':
        print("No time file provided, all files will be assigned the same timestamp corresponding to the time of the analysis execution")
        now = datetime.now()
    ...
    for i in file_numbers:
        ...
        progress = (i - file_numbers[0]) / (file_numbers[-1] - file_numbers[0] + 1) * 100
        print(f"Processing file {i} ({progress:.2f}%)...", end="\r", flush=True)

        filename = f"{data_folder}/0{i}.lstor"
        ...
        try:
            ...
            # check if file exists before opening (otherwise throw excecution that stops the whole process instead of just skipping the file)
            ...
            with open(filename, "r") as f:
                ...
                while True:
                    ...
                    try:
                        ch  = 31 - int(parts[5]) if self.channel_inversion else int(parts[5])
                        aa  = int(parts[7])
                        ab  = int(parts[8])
                        bb  = ab - aa
    ...
    # Stack into 3D array
    # -----------------------------
    bg_data_ab  = np.stack(all_hist_ab, axis=0)  # (time, ene, ch)
    bg_data_aa  = np.stack(all_hist_aa, axis=0)  # (time, ene, ch)
    bg_data_bb  = np.stack(all_hist_bb, axis=0)  # (time, ene, ch)
    bg_data_pos = np.stack(all_hist_pos, axis=0) # (time, pos, ch)
    bg_data_ene = np.stack(all_hist_ene, axis=0) # (time, ene, pos, ch)

    # -----------------------------
    # Sort by time
    # -----------------------------
    idx = np.argsort(global_time_list)
    bg_data_ab  = bg_data_ab[idx]
    bg_data_aa  = bg_data_aa[idx]
    bg_data_bb  = bg_data_bb[idx]
    bg_data_pos = bg_data_pos[idx]
    bg_data_ene = bg_data_ene[idx]
    global_time_list = np.array(global_time_list)[idx]

    return bg_data_ab, bg_data_aa, bg_data_bb, bg_data_pos, bg_data_ene, global_time_list 
...
class CSPEC(ndet_lib.ndet):
    ...
    def importListMode(self, file_numbers, time_file, data_folder):
        ab, aa, bb, pos, gain, time_list = import_nomad_list(self, file_numbers, time_file, data_folder)
